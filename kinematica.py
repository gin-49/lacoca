...
max_required_speed = max(np.max(np.abs(motor_speeds1)), np.max(np.abs(motor_speeds2)))
...
ani = FuncAnimation(fig, update, frames=len(vPx), interval=delta_t * 1000, blit=True)
plt.show()
...
for i in range(len(vMotor1Steps)):
    print(
        f"Step {i}: θ1={np.degrees(vTheta1[i]):.2f}°, θ2={
            np.degrees(vTheta2[i]):.2f}°, "
